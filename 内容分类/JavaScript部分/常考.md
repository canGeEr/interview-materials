...
- 函数
  ...
  - 箭头函数
    ...
    - 没有 arguments
    - 在 class 关键字里，为 class 属性成员绑定的箭头函数是在实例对象上的，而不是原型上的（其实本身这个写法就算这个意义）
...
函数内部可以定义子函数，子函数可以被暴露到父函数之外使用，子函数在声明的时候预先进行一遍扫描（函数声明预解析），将内部的访问父函数的变量，复制打包给该函数，并保存在 closure属性上，因此当父函数执行结束退出的时候，即使栈空，子函数依然能够正确的访问“父函数的变量”
